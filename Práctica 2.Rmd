...
```{r}
v<-numeric(3)
v # el vector tiene longitud 3 y sus componentes serán NA (Not Available/"Missing" Values) que es la forma como R maneja los datos omitidos o faltantes.
```
-   Ejemplo 2:
...
-   Ejemplo 2:
```{r}
s2 = seq(from=-1, to=5)#crea un vector cuyo elemento inicial es -1 y su elemento final es 5, y cada dos elementos consecutivos del vector tienen una diferencia de una unidad.
s2
...
-   Ejemplo 1:
```{r}
1/x #observe que calcula el inverso de cada elemento del vector
```
-   Ejemplo 2:
...
elemento a elemento, el vector 2*x repetido 2.2 veces, el vector y, y el número 1 repetido 11 veces “Reciclado en R es repetir las veces necesarias un vector cuando en una operación intervienen vectores de distinta longitud”).
...
calcula el producto interno entre dos vectores. Ambos deben tener el mismo número de elementos.
...
```{r}
etiqs<-paste(c("X", "Y"), 1:10, sep="") # Crea un vector de caracteres resultado de la unión de “X” o de “Y” con uno de los número comprendidos entre 1 y 10, sep=”” indica que no se deja espaciado en la unión.
etiqs
...
```{r}
M[2,3]<-6 #similar a la de un vector pero considerando que deben ¡ utilizarse dos índices para indicar fila y columna
M
```
...
Observe que R almacena los elementos por columna. Se pueden explorar algunas características de la
